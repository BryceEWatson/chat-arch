...
| G4 | Curator ranker uses correlation only as tie-breaker within a tier (does NOT promote across tiers) | in-review | PR #TBD (`curatorRanker.crossTierInvariant.test.ts` — discoverable named test pinning three failure modes: correlation folded into composite, tier-vs-composite sort order, cross-tier tie-break. The invariant itself was already implemented + tested in F3/PR #85; G4 is the discoverable cross-ref.) |
| G5 | Falsifier runs permutation test resampling project sessions to confirm Δ unlikely under H0 | in-review | PR #TBD (`permutationTestDelta` in `packages/analysis/src/correlationPermutation.ts` — pure, deterministic xorshift32-seeded permutation kernel; two-sided empirical p-value clamped to `[1/(K+1), 1]`; defensive contract handles empty/NaN/constant-pool. 16 tests cover correctness + bounds + determinism + Welch-vs-permutation consistency.) |
