...
| # | Sub-task | Status | PR / notes |
|---|---|---|---|
| H1 | Standalone MCP server package under `packages/mcp-server/` | in-review | PR #TBD (`packages/mcp-server/` workspace package + `index.ts` barrel + `createMcpServer` factory + integration with vitest workspace; depends on `@chat-arch/exporter` for the future SDK wiring in H3) |
| H2 | Read-only by default; narrow tool surface (no arbitrary `readFile`, no `claude -p` exec from server, working-dir scoped to `chat-arch-data/`) | in-review | PR #TBD (`workingDir.ts` enforces basename = `chat-arch-data` + absolute-path requirement + `assertPathWithinWorkingDir` traversal guard with trailing-sep boundary check; `readOnly.ts` allow-listed verb prefixes + deny-list defense-in-depth + frozen `READ_ONLY_POLICY` export; `server.ts` calls both at registration time so write-shaped tool names fail before reaching the protocol layer. 52 tests cover the policy invariants; protocol layer + SDK tool registration land with H3.) |
