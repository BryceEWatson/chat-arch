...
| # | Sub-task | Status | PR / notes |
|---|---|---|---|
| I1 | Update project `CLAUDE.md` "Data on disk" section: new SQLite DB file + renamed entity-states ledger + Narrative schemaVersion 2 PII expansion | in-review | PR #TBD (CLAUDE.md "SQLite substrate" section enumerates the entity-table PII categories — narratives `intent`/`observation`/`inference` columns + `narrative_evidence` turn citations + `patterns` body + `findings` payloadJson + `sessions`/`messages`/`revisions` raw transcript content. Adds Rev3-H MCP-server read-only surface note. Also documents the legacy `entity-states.json` → SQLite migration path.) |
| I2 | Update README to formalize hosted-viewer divergence (chat-arch.dev stays JSON-sidecar-only as deliberately-scoped demo of the local pipeline) | in-review | PR #TBD (README "Hosted vs local — deliberately scoped divergence" subsection with a capability table contrasting chat-arch.dev (demo only) against local `pnpm dev` (SCAN LOCAL + /api/* + SQLite + curator + falsifier + MCP). Pins that the hosted demo is JSON-sidecar-only by design.) |
| I3 | Extend `.githooks/pre-commit` to scan any new sidecar with user-content (Narratives carry quoted excerpts → PII) | in-review | PR #TBD (3 new rules: Rule 0 blocks any `*.db`/`*.db-wal`/`*.db-shm`/`*.sqlite*` staging anywhere in tree; Rule 1 blocks any staged file under `apps/standalone/public/chat-arch-data/` other than the empty baseline manifest; Rule 2 blocks any staged file under `apps/standalone/chat-arch-data/` (SQLite sibling dir). Existing manifest baseline-integrity checks preserved as Rule 3.) |
| I4 | Confirm `.gitignore` covers SQLite DB family + any new sidecars | in-review | PR #TBD (audit confirmed prior coverage of `*.db*` + `apps/standalone/chat-arch-data/` + `apps/standalone/public/chat-arch-data/*`. Added explicit documentation entries for the two Rev3-F sidecars `analysis/curator-feed.json` + `analysis/falsifier-verdicts.json` (already covered by the wildcard but listed for auditability per the existing convention).) |
| I5 | Bump `EXPORTER_VERSION` to reflect the SQLite cutover; add CHANGELOG entry | in-review | PR #TBD (1.2.0 → 1.3.0 in `packages/exporter/src/analysis/index.ts` with an inline bump-rationale comment; CHANGELOG.md gains a `[1.3.0] — 2026-05-24` block summarizing the full Rev3 build phase-by-phase (A through I) with the on-disk artifact set deltas.) |
| I6 | Manual hygiene check: fresh contributor can read CLAUDE.md and correctly enumerate what's on disk and what carries PII | in-review | PR #TBD (CLAUDE.md "Fresh-contributor hygiene check" subsection answers the 6 canonical questions: what's on disk that I should never commit, what PII categories live in the SQLite tables, where the ledger can be read from, which analysis/ sidecars carry PII vs aggregate-only, hosted vs local divergence, when to bump EXPORTER_VERSION. Closes Phase Rev3-I.) |
