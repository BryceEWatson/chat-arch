...
| A11 | Seeded-fixture test corpus → SDK returns expected rows | complete-and-merged | PR #60 — closes Phase Rev3-A |
...
| # | Sub-task | Status | PR / notes |
|---|---|---|---|
| B1 | Extend `packages/schema/src/narrative.ts` with provenance fields (intent, observation, inference, attributedTo, verifiedAt, confidence, supportingCount, contradictingCount, correlatedOutcome, schemaVersion) | complete-and-merged | PR #66 |
| B2 | Extend `NarrativeEvidence` with optional `turnIndex` | complete-and-merged | PR #66 |
| B3 | DB migration adding the new columns to the `narratives` table | complete-and-merged | PR #66 (migration `002-narrative-provenance.ts`) |
| B4 | `validateNarrative()` accepts both schemaVersion 1 and 2 | complete-and-merged | PR #66 |
| B5 | Backfill kernel — one-shot run over any existing narratives (post zero-data start, normally empty) computing confidence + setting `attributedTo='deterministic'` + bumping to schemaVersion 2 | complete-and-merged | PR #68 (`backfillNarrativeProvenance.ts` kernel; wired into `getChatArchDb()` init in the exit-review cleanup PR — was previously dead code per the final review-loop) |
| B6 | Confidence-ladder helper (`computeConfidence(supporting, contradicting, prior)`) consuming THRESHOLDS | complete-and-merged | PR #67 |
| B7 | Calibration fail-safe: kernels with `calibrationCompletedAt = null` get effective prior pinned to `narrativeRung.uncalibratedPrior`; banner surfaces "kernel X uncalibrated" | complete-and-merged | PR #67 |
| B8 | Named test `narrative.migration.test.ts` — covers legacy parse, deterministic backfill, round-trip, dual-schema acceptance | complete-and-merged | PR #66 + #68 (dual-schema acceptance + structural rejections in #66; backfill round-trip in #68) |
| B9 | PII handling for narrative previews — default-blur with reveal-on-click before any curator surface ships | complete-and-merged | PR #69 — closes Phase Rev3-B (BlurredPii component + wired into ProjectsMode narrative title/body; evidence pill `title` attribute leak fixed) |
...
| D4 | Shelved-permanently affordance + explicit "show shelved" UI toggle | complete-and-merged | PR #78 (transient `showShelved` state + checkbox in `ProjectDetail`'s narratives header; partitions narratives via `narrativeSaturation`; safe default = OFF) |
| D5 | Tests: cap-K behavior + family-wise α inflation documented in methodology disclosure | complete-and-merged | PR #78 — closes Phase Rev3-D (gate test walks `dismissalCount` from cap−1 → cap and asserts shelved transition; `MethodologyDisclosure` gains family-wise correction caveat) |
...
| E4 | Next-sessions watcher: triggers `RECURRING_AFTER_APPLIED` or `WATCH_INCONCLUSIVE` on N=5 / 60d / project-inactivity 30d | complete-and-merged | PR #81 (`evaluateAppliedPatternWatcher` pure-decision kernel in `packages/analysis/src/applyWatcher.ts`; emits `{kind:'open'\|'holding'\|'recurring'\|'inconclusive'}` verdicts. Orchestration lands with Rev3-F curator) |
| E5 | Wall-clock timeout emits low-priority `WATCH_INCONCLUSIVE` Narrative (not silence) | complete-and-merged | PR #81 (same kernel returns `{kind:'inconclusive', reason:'wall-clock-timeout'\|'project-inactive'}`; curator formats the Narrative at low feed priority) |
| E6 | Tests: applied pattern visibly closes its watcher within the window; bypass path produces auditable Pattern row | complete-and-merged | PR #82 (closes Phase Rev3-E — integration test in `apps/standalone/test/integration/closure-c-applied-pattern-gate.test.ts` composes E3 encode → E4+E5 watcher kernel across all three closure paths: holding, recurring, inconclusive) |
...
| # | Sub-task | Status | PR / notes |
|---|---|---|---|
| F1 | `/curate` skill scaffolded under `.claude/skills/` driven by `claude -p` via `resolveClaude.ts` | complete-and-merged | PR #83 (SKILL.md at `.claude/skills/curate/` — frontmatter + 4-stage pipeline scaffold referencing F3 ranker kernel + F4 falsifier hand-off + F8 meta-validation + F9 PRACTICE surface) |
| F2 | Falsifier skill — structurally separate, different system prompt, different prompt template | complete-and-merged | PR #83 (SKILL.md at `.claude/skills/falsify/` — separate agent type, neutral auditor framing; per-finding verdict pipeline with self-consistency K=3 + atomic verdict write; never re-ranks or composes with generator) |
| F3 | Curator ranks tier-2 + tier-3 Narratives, outcome-correlation as tie-breaker only | complete-and-merged | PR #85 (`rankCuratorCandidates` pure-decision kernel in `packages/analysis/src/curatorRanker.ts`; tier-bucket-first sort prevents cross-tier promotion via correlation; correlation honored only as within-tier tie-breaker per plan §G2) |
| F4 | Falsifier verifies each finding's `evidenceChain` cites real session turns whose content supports the claim | complete-and-merged | PR #85 (`aggregateFalsifierVerdicts` aggregator in `packages/analysis/src/falsifierVerifier.ts`; per-turn LLM verdicts in `/falsify` skill, aggregated here against `THRESHOLDS.curator.falsifierMinSupportRatio` = 0.6; unavailable counts as failure for citation hygiene) |
| F5 | Subprocess fallback: `claude --version` probe at startup; 429 backoff + retry once; banner on persistent failure | complete-and-merged | PR #84 (`probeClaudeAvailable` + `runCuratorSubprocess` in `apps/standalone/src/lib/curatorClaude.ts`; 1.5s probe timeout; 50ms→1s exp backoff with `maxElapsedMs=30s`; tagged-union `RunCuratorResult` for banner dispatch) |
| F6 | API-key fallback OFF by default; `chatArchCuratorApiKeyOptIn` localStorage toggle in local viewer only | complete-and-merged | PR #84 (two-rail default-deny: viewer `curatorApiKeyOptIn.ts` + server `apiKeyFallbackAllowedFromEnv` env-flag; both must be ON for the env var to reach the subprocess; otherwise `ANTHROPIC_API_KEY` is scrubbed before spawn) |
...
| F8 | Meta-validation: rolling 4-week n=40 verdicts; Wilson lower bound against `curator.falsifierAccuracyFloor`; banner on drift | complete-and-merged | PR #86 (`evaluateFalsifierMetaAccuracy` pure-decision kernel in `packages/analysis/src/falsifierMetaAccuracy.ts`; sample-size guard (n ≥ 40) prevents small-n false-alarm noise; Wilson LB via existing `wilsonCI` helper; emits `{inDrift, n, accuracy, lowerBound, floor, minN}` for the curator banner) |
| F9 | Curator feed surfaces as top section on PRACTICE (above the four lenses), NOT a new top-level surface | complete-and-merged | PR #87 (`CuratorFeed` component + `curatorFeedClient.loadCuratorFeed`; renders ranked items with kind chip + composite score + falsifier-status tag + F8 drift banner; graceful empty state when `analysis/curator-feed.json` is absent — /curate skill hasn't run yet) |
| F10 | Tests: falsifier rejection rate inside bracket; meta-accuracy stable on fixture corpus | complete-and-merged | PR #88 — closes Phase Rev3-F (`curatorPipeline.gate.test.ts` composes F3 ranker + F4 verifier + F8 meta-validation on synthetic fixtures; pins rejection rate at 0.35 inside `[0.2, 0.5]` bracket, meta-accuracy stable in high-accuracy regime (n=40@95% LB clears floor; n=100@90% likewise), drift correctly fires at 60% accuracy. User-engagement clause from plan is environmental, not testable in CI. Closes Phase Rev3-F.) |
...
| # | Sub-task | Status | PR / notes |
|---|---|---|---|
| G1 | Welch's t-test (or non-parametric permutation) implementation in `packages/analysis/src/` | complete-and-merged | PR #89 (`welchsTTest` in `packages/analysis/src/welchsTTest.ts` — pure function returning `{t, delta, standardError, degreesOfFreedom, pValueTwoSided, valid}`; defensive on degenerate inputs; normal-CDF p-value approximation reuses existing `normalCdf` helper) |
| G2 | Outcome-correlation tag visibility gated on `\|Δ\|/SE` exceeding `curator.outcomeCorrelationSignificance` AND `evidence.length ≥ 5` | complete-and-merged | PR #89 (`evaluateCorrelationTagVisibility` in `packages/analysis/src/correlationTagGate.ts` — tagged-union verdict (`visible \| 'insufficient-evidence' \| 'below-significance' \| 'invalid-stat'`) so renderers can show distinct copy per non-shown reason) |
| G3 | Extend `SourceAttribution.tsx` `AttributionKind` union with new rungs (`'tier1' \| 'tier2' \| 'tier3' \| 'falsifier-verified' \| 'llm-derived' \| 'deterministic-with-prior' \| 'correlation-significant'`) | complete-and-merged | PR #90 (7 new rungs added; `REV3_ATTRIBUTION_KINDS` const exports the subset; existing 9 tests preserved + 3 new tests pin the new rungs render + aria + REV3 constant) |
...
| G6 | Tests: correlation tags visible only when |Δ|/SE exceeds threshold; tie-breaker only fires on evidence ≥ 5 | complete-and-merged | PR #92 (`correlationGate.gate.test.ts` — integration gate composing G1 Welch + G2 visibility + F3/G4 ranker + G5 permutation back-stop. Pins both invariants via swap-test: with G2 hiding the tag (cited.length < 5 OR \|t\| below significance), correlation cannot influence rank order; with G2 passing, correlation tie-break decides. 12 tests across visibility / tie-breaker / permutation-Welch agreement. Iter-1 of review-loop fixed a load-bearing semantic mismatch: G2's `evidenceLength` contract is "rows the candidate cites" = `cited.length` only, NOT `cited+uncited`. Closes Phase Rev3-G.) |
...
| # | Sub-task | Status | PR / notes |
|---|---|---|---|
| H1 | Standalone MCP server package under `packages/mcp-server/` | complete-and-merged | PR #93 (`packages/mcp-server/` workspace package + `index.ts` barrel + `createMcpServer` factory + integration with vitest workspace) |
| H2 | Read-only by default; narrow tool surface (no arbitrary `readFile`, no `claude -p` exec from server, working-dir scoped to `chat-arch-data/`) | complete-and-merged | PR #93 (`workingDir.ts` + `readOnly.ts` + registration-time policy gate). Iter-1 hardening (per adversarial review): readOnly segment-scan rejects embedded write verbs (e.g. `list_then_delete_project`); workingDir normalizes Windows drive-letter case + rejects UNC paths; server adds `closed` flag + deep-freezes registered tools. |
| H3 | Expose data SDK query methods as MCP tools (projects, topics, narratives, patterns, findings) | complete-and-merged | PR #94 (`tools.ts` — `registerSdkTools(server, {db})` wires 10 tools: get_/list_ for each of 5 entities + minimal hand-rolled arg validators (unknown-key rejection, type+presence checks). Filter shapes mirror `@chat-arch/exporter/db` `ListNarrativesFilter` / `ListPatternsFilter` / `FindingsFilter`. Also extends `@chat-arch/exporter/db` SDK barrel with `seedRev3Fixture` + `SEED_IDS` re-exports so the H5 gate test can share the canonical fixture without duplicating ~50 row inserts.) |
| H4 | Localhost-bind only in v2.0; remote MCP-over-HTTP explicit non-goal (descoped to later amendment) | complete-and-merged | PR #94 (`localhostBind.ts` — `assertLocalhostBind({kind, host?, port?})` policy gate. Tagged-union `BindDescriptor` for the future transport-layer call site; tcp bind restricted to loopback literals `127.0.0.1`/`::1`/`::ffff:127.0.0.1`/`localhost` (case-sensitive); port required + integer in [1, 65535]; stdio binding accepts no host/port. Frozen `LOCALHOST_BIND_POLICY` export. 24 tests covering acceptance, rejection (non-loopback / invalid port / wrong case / DNS deception), and the policy export shape.) |
| H5 | Tests: same query returns equivalent results in the viewer and from an external claude session | complete-and-merged | PR #94 — closes Phase Rev3-H scaffolding (`tools.gate.test.ts` — seeds in-memory DB via `seedRev3Fixture`, registers the 10 H3 tools, calls each through the MCP `tool.handler(args)` surface, asserts deep-equal vs direct SDK call against the same DB handle. 20 tests cover get-by-id + unfiltered-list + filtered-list + arg-shape rejection + tool inventory + read-only-allowlist sanity. The "viewer" half of the plan's exit phrase is the SDK call (viewer reads via SDK); the "external claude session" half is the tool handler — equivalent if tool layer adds no transformation. **DEFERRED to a follow-on protocol PR (tracked in plan §Deferred):** the stdio / `@modelcontextprotocol/sdk` transport wiring that actually lets an external claude session reach the tool surface; until that lands the end-to-end exit phrase is unverified.) |
| H6 | Stdio transport + `@modelcontextprotocol/sdk` wiring + true end-to-end equivalence gate (external claude session → MCP protocol → tool handler) | dropped | Deferred to follow-on PR after Rev3 closes — see plan §Deferred work. The H5 in-process gate covers the wiring contract; H6 will cover the protocol-roundtrip contract. |
...
| # | Sub-task | Status | PR / notes |
|---|---|---|---|
| I1 | Update project `CLAUDE.md` "Data on disk" section: new SQLite DB file + renamed entity-states ledger + Narrative schemaVersion 2 PII expansion | complete-and-merged | PR #95 (CLAUDE.md "SQLite substrate" section enumerates the entity-table PII categories — narratives `intent`/`observation`/`inference` columns + `narrative_evidence` turn citations + `patterns` body + `findings` payloadJson + `sessions`/`messages`/`revisions` raw transcript content. Adds Rev3-H MCP-server read-only surface note. Also documents the legacy `entity-states.json` → SQLite migration path.) |
| I2 | Update README to formalize hosted-viewer divergence (chat-arch.dev stays JSON-sidecar-only as deliberately-scoped demo of the local pipeline) | complete-and-merged | PR #95 (README "Hosted vs local — deliberately scoped divergence" subsection with a capability table contrasting chat-arch.dev (demo only) against local `pnpm dev` (SCAN LOCAL + /api/* + SQLite + curator + falsifier + MCP). Pins that the hosted demo is JSON-sidecar-only by design.) |
| I3 | Extend `.githooks/pre-commit` to scan any new sidecar with user-content (Narratives carry quoted excerpts → PII) | complete-and-merged | PR #95 (3 new rules: Rule 0 blocks any `*.db`/`*.db-wal`/`*.db-shm`/`*.sqlite*` staging anywhere in tree; Rule 1 blocks any staged file under `apps/standalone/public/chat-arch-data/` other than the empty baseline manifest; Rule 2 blocks any staged file under `apps/standalone/chat-arch-data/` (SQLite sibling dir). Existing manifest baseline-integrity checks preserved as Rule 3.) |
| I4 | Confirm `.gitignore` covers SQLite DB family + any new sidecars | complete-and-merged | PR #95 (audit confirmed prior coverage of `*.db*` + `apps/standalone/chat-arch-data/` + `apps/standalone/public/chat-arch-data/*`. Added explicit documentation entries for the two Rev3-F sidecars `analysis/curator-feed.json` + `analysis/falsifier-verdicts.json` (already covered by the wildcard but listed for auditability per the existing convention).) |
| I5 | Bump `EXPORTER_VERSION` to reflect the SQLite cutover; add CHANGELOG entry | complete-and-merged | PR #95 (1.2.0 → 1.3.0 in `packages/exporter/src/analysis/index.ts` with an inline bump-rationale comment; CHANGELOG.md gains a `[1.3.0] — 2026-05-24` block summarizing the full Rev3 build phase-by-phase (A through I) with the on-disk artifact set deltas.) |
| I6 | Manual hygiene check: fresh contributor can read CLAUDE.md and correctly enumerate what's on disk and what carries PII | complete-and-merged | PR #95 — closes Phase Rev3-I (CLAUDE.md "Fresh-contributor hygiene check" subsection answers the 6 canonical questions: what's on disk that I should never commit, what PII categories live in the SQLite tables, where the ledger can be read from, which analysis/ sidecars carry PII vs aggregate-only, hosted vs local divergence, when to bump EXPORTER_VERSION. Closes Phase Rev3-I.) |
