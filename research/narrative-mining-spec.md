...
- `projectId` sanitization: when present, MUST match `/^[a-zA-Z0-9](?:[a-zA-Z0-9_.-]{0,126}[a-zA-Z0-9])?$/` (forbids leading/trailing `-`, `_`, or `.` to block argv-flag-shaped values like `--evil`; min 1 char, max 128 chars; allows internal `.` for namespace-style project ids like `proj.shopforge`) AND MUST exist in the current `manifest.json`'s project list. Reject 400 on either failure.
...
- **`narrativeRung.test.ts` extension** — extend existing tests for `narrativeTier()` with the new V1-cap opts: assert `attributedTo === 'llm-derived' && contradicting === 0 && verifiedAt === null` clamps tier to ≤ 2 regardless of confidence; assert exemption requires BOTH `verifiedAt !== null` AND `contradicting > 0` (just one is not enough); assert legacy callers without the opts param behave identically to today (back-compat).
...
- **`/api/mine-persona` projectId sanitization hardening.** The persona endpoint inherits the same allow-list gap closed in this V1 for `/api/mine-narratives`. Apply the same `/^[a-zA-Z0-9_-]{1,128}$/` + manifest-membership check to `mine-persona.ts` in a follow-up — out of scope for the narrative-mining PR but documented here so the hole isn't forgotten.
...
| LLM budget proxy? | **1200 candidates — placeholder, pre-launch** | `0.8 × persona's 1500` for ~25% richer per-row token cost (title + previewExcerpt + summaryExcerpt + outcomeMarkers vs persona's single excerpt). Pre-launch placeholder, NOT a derivation identity. Calibrate after first 10 runs against actual Stage-2 USD-per-candidate. |
...
| V1 tier-cap on LLM rows? | **Yes — fold into `narrativeTier()` signature, cap to tier ≤ 2 when `attributedTo === 'llm-derived' && contradictingCount === 0 && verifiedAt == null`** | Embedding in `narrativeTier` preserves the file's "single point of truth" invariant; no sibling helper, no cap-bypass risk for future callers. |
| Falsifier cap exemption? | **Predicate requires BOTH `verifiedAt !== null` AND `contradictingCount > 0`** | Falsifier verifies citation hygiene, NOT corpus-wide contrary evidence. V1's hardcoded `contradicting=0` means the exemption is inactive even for verified rows; V1.1's contrary-finder activates it. The exemption clause is wired now so V1.1 drops the cap by deleting the clause, not threading new logic. |
