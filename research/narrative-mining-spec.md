...
V1 mitigates with **compare-and-swap on rescan timestamp**: the skill captures `narratives.json`'s `generatedAt` at T1, re-reads just before the write at T3, and aborts-with-retry if `generatedAt` changed. One retry; on second mismatch, surface as `status: 'concurrent-rescan-aborted'` in the status file and skip the project. The retry consumes additional `maxLlmUsdPerProject` budget within the CURRENT scan (Stage 2's bucket sub-agents + synthesis sub-agent re-fire). If the retry pushes the project over the cap, the next Stage-2 sub-agent dispatch surfaces a `budget-exceeded` skip-row via the normal Stage-2 budget gate — no special CAS-vs-budget arbitration logic is needed. A project that keeps losing CAS races persistently is a deployment problem the user should see via the status file's `concurrent-rescan-aborted` row, not a budget violation to police via custom retry-bookkeeping. (Earlier drafts said "if a CAS retry would push the project over `maxLlmUsdPerProject`, write `concurrent-rescan-aborted` without retrying" — but the skill cannot reliably introspect its prior LLM spend from within `claude -p`, so that budget-aware retry decision is not implementable in V1. The §"Stage 2c" execution rule below is authoritative.) V1.1 promotes this to a real shared-write-lock if observed in the field. The CAS is documented in `Stage 2c` execution rules.
...
- **Hand-authored narrative exemplar (analog to `bryce.md`).** Bryce did not author one; narrative-mining V1 ships without a hand-authored ground-truth benchmark for the narrative domain specifically. The 160-session number that appears in `bryce.md`'s "Files scanned" line is a PERSONA-domain artifact (chat-arch CLI sessions sampled for persona authoring) — it is NOT a narrative-domain anchor, and the V1 `maxSessionsForCorpus=200` is a structural mirror of the persona threshold, not a derivation from any narrative-domain prototype. The heuristic kernel's existing 9 narratives are the soft-prior baseline that LLM narratives must improve on. Calibration after first run substitutes for the missing exemplar.
...
5. `THRESHOLDS.narrative.maxSessionsForCorpus = 200` — inherits `THRESHOLDS.persona.maxSessionsForCorpus`'s value without re-deriving it; the persona threshold's empirical existence proof is the 160-session bryce.md prototype, and narrative-mining mirrors the persona threshold so the 4-quartile-by-recency sampler behaves identically across both pipelines (50 sessions per bucket). Refit only if the persona threshold changes, OR if calibration evidence shows the Stage-2 LLM ceiling on input handling has shifted. Treated as a structural alignment, NOT a free-tuning placeholder.
6. `THRESHOLDS.narrative.evidenceMinPerNarrative = 2` — single-session "narratives" are anecdotes, not durable themes (iter-1 persona stat-rigor finding). Load-bearing structural floor aligned with `THRESHOLDS.narrativeRung.tier2SupportingMin = 2`; refit only in lockstep with the tier-2 supporting-count gate. NOT a free-tuning placeholder. (Cross-reference NOT pinned in code; spec-only invariant.)
7. `THRESHOLDS.narrative.maxCandidatesPerRecencyBucket = 300` — per-bucket defensive ceiling. Currently unreachable under the upstream `maxSessionsForCorpus=200` sampler which yields ≤50 candidates per bucket AND under `buildProjectCandidates`'s current "1 candidate per session" emission policy. The cap engages only when `maxSessionsForCorpus` is raised past ~1204 OR when a future change emits multiple candidates per session. Forward-compat guard; refit alongside `maxSessionsForCorpus` if the sampler ceiling moves, or alongside the candidate-per-session policy if it changes.

**Calibration ownership.** Bryce is the V1 calibration owner for all rows above. A future contributor taking a row should append `(owner: <name>)` to the row's prose. No-owner rows are revisited only on user-reported issue.
