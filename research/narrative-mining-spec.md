...
5. `THRESHOLDS.narrative.maxSessionsForCorpus = 200` — mirrors `THRESHOLDS.persona.maxSessionsForCorpus` (the smallest multiple of 4 that gives a clean even per-quartile split AND is at least the persona prototype's 160-session author run). Refit only if the persona threshold changes, OR if calibration evidence shows the Stage-2 LLM ceiling on input handling has shifted. Treated as a structural alignment, NOT a free-tuning placeholder.
